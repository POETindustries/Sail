...
const metaKeys = "title,keywords,description,language,page_topic,revisit_after,robots"
...
func (m *Meta) ScanFromDB(conn *dbase.Conn) bool {
	data := conn.MetaData(metaKeys, m.ID)

	if data.Scan(&m.Title, &m.Keywords, &m.Description, &m.Language,
		&m.PageTopic, &m.RevisitAfter, &m.Robots) != nil {
		return false
	}
